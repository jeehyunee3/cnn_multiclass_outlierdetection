...
#2. Data clustering ; GMM Clustering

#2.1 Creating and training the clustering algorithm
dimen=6
gmm = GaussianMixture(n_components = dimen, random_state = 0)
gmm_labels = gmm.fit_predict(img_png_2d)

#2.2 Printing the results by class.
train_df.loc[2]['label']
train_df['label']
np.unique(train_df['label'])
print(np.unique(gmm_labels.labels_, return_counts=True))
...
result_set_list = [A, B, C, D, E, F]
# seperate data
idx = 0
for x in gmm_labels.labels_ :
